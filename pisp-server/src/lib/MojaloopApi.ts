...
export default class MojaloopApi {
...
/* --- Existing Mojaloop Calls --- */
   ...
   */
  static async getParty(opts: any): Promise<any> {
   ...
   * 
   */
  static async startTransfer(opts: any): Promise<any> {
   ...
   *
   */
  static async approveParty(transferId: string, opts: any): Promise<any> {
    const options = {
   ...
   *
   */
  static async approveQuote(transferId: string, opts: any): Promise<any> {
    const options = {
      url: `http://${Config.SCHEME_ADAPTER_ENDPOINT}/transfers/${transferId}`,
      method: 'PUT',
      json: true, 
